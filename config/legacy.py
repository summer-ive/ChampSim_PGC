...
branch_variant_data = [
    ...
    (
        "last_branch_result",
        (("uint64_t", "ip"), ("uint64_t", "target"), ("uint8_t", "taken"), ("uint8_t", "branch_type")),
        "void",
    ),
...
btb_variant_data = [
    ...
    (
        "update_btb",
        (("uint64_t", "ip"), ("uint64_t", "predicted_target"), ("uint8_t", "taken"), ("uint8_t", "branch_type")),
        "void",
    ),
...
pref_variant_data = [
    ...
    (
        ...
        (("uint64_t", "ip"), ("uint8_t", "branch_type"), ("uint64_t", "branch_target")),
        "void",
    ),
]
...
def get_discriminator(variant_data, module_data):
    ...
    yield "constexpr"
    yield from cxx.function(
        f"{classname}::has_function",
        ["return std::string_view{CHAMPSIM_LEGACY_FUNCTION_NAMES}.find(name) != std::string_view::npos;"],
        rtype="bool",
...
if __name__ == "__main__":
    ...
    infos = [{"name": modules.get_module_name(p), "path": p, "legacy": True} for p in args.paths]
    for i in infos:
        i.update(
            {
                "type_guess": next(
                    filter(lambda t: t in i["path"], ("branch", "btb", "prefetcher", "replacement")), ""
                ),
    ...
    if args.kind == "mangle":
        fileparts.extend(
            (
                os.path.join(mod_info["path"], "legacy_bridge.inc"),
                filewrite.cxx_file(
                    (
    ...
    if args.kind == "header":
        fileparts.extend(
            (
                os.path.join(mod_info["path"], "legacy_bridge.h"),
                filewrite.cxx_file(
                    (
                        "#include <string_view>",
                        '#include "modules.h"',
    ...
    if args.kind == "source":
        fileparts.extend(
            (
                os.path.join(mod_info["path"], "legacy_bridge.cc"),
                filewrite.cxx_file(
                    (
