...
bool CACHE::try_hit(const tag_lookup_type& handle_pkt)
{
  ...
  if (should_activate_prefetcher(handle_pkt)) {
    ...
    metadata_thru =
        impl_prefetcher_cache_operate(handle_pkt.cpu, module_address(handle_pkt), v_addr, handle_pkt.ip, hit, useful_prefetch, handle_pkt.type, metadata_thru);
  }
...
uint32_t CACHE::impl_prefetcher_cache_operate(uint32_t trigger_cpu, champsim::address addr, champsim::address v_addr, champsim::address ip, bool cache_hit,
                                              bool useful_prefetch, access_type type, uint32_t metadata_in) const
{
  return pref_module_pimpl->impl_prefetcher_cache_operate(trigger_cpu, addr, v_addr, ip, cache_hit, useful_prefetch, type, metadata_in);
}
