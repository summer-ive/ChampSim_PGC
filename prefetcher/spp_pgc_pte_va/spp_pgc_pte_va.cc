...
#include "spp_pgc_pte_va.h"

void spp_pgc_pte_va::prefetcher_initialize()
{
  std::cout << "Prefetcher: spp_pgc_pte_va" << std::endl;
...
void spp_pgc_pte_va::reset_roi_status()
{
...
bool spp_pgc_pte_va::is_adjacent_in_virtual(uint32_t trigger_cpu, champsim::page_number trigger_vpage, champsim::page_number pf_ppage)
{
  bool is_allocated;
...
void spp_pgc_pte_va::prefetcher_cycle_operate() {}

uint32_t spp_pgc_pte_va::prefetcher_cache_operate(uint32_t trigger_cpu, champsim::address trigger_paddr, champsim::address trigger_vaddr, champsim::address ip,
                                                  bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  typename spp_pgc_pte_va::offset_type::difference_type delta = 0;
  std::vector<typename spp_pgc_pte_va::offset_type::difference_type> delta_q(intern_->MSHR_SIZE);
  ...
  FILTER.check(trigger_paddr, spp_pgc_pte_va::L2C_DEMAND);
  ...
  do {
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      ...
      if (confidence_q[i] >= PF_THRESHOLD) {
        ...
        if (FILTER.check(pf_paddr, (is_prefetch_in_this_level ? spp_pgc_pte_va::SPP_L2C_PREFETCH : spp_pgc_pte_va::SPP_LLC_PREFETCH))) {
          ...
          if (!is_pgc_enabled && is_pgc_candidate) {
            if constexpr (GHR_ON) {
              ...
              GHR.update_entry(curr_sig, confidence_q[i], spp_pgc_pte_va::offset_type{pf_paddr}, delta_q[i]);
            }
            continue;
          }
...
uint32_t spp_pgc_pte_va::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr,
                                               uint32_t metadata_in)
{
  if constexpr (FILTER_ON) {
    ...
    FILTER.check(evicted_addr, spp_pgc_pte_va::L2C_EVICT);
  }

  return metadata_in;
}

void spp_pgc_pte_va::prefetcher_final_stats()
{
...
uint64_t spp_pgc_pte_va::get_hash(uint64_t key)
{
...
void spp_pgc_pte_va::SIGNATURE_TABLE::read_and_update_sig(champsim::address addr, uint32_t& last_sig, uint32_t& curr_sig,
                                                          typename offset_type::difference_type& delta)
{
  auto set = get_hash(spp_pgc_pte_va::unit_number{addr}.to<uint64_t>()) % ST_SET;
  auto match = ST_WAY;
...
void spp_pgc_pte_va::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
...
void spp_pgc_pte_va::PATTERN_TABLE::read_pattern(uint32_t curr_sig, std::vector<typename offset_type::difference_type>& delta_q,
...
bool spp_pgc_pte_va::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
  ...
  case spp_pgc_pte_va::SPP_L2C_PREFETCH:
  ...
  case spp_pgc_pte_va::SPP_LLC_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      if constexpr (SPP_DEBUG_PRINT) {
        std::cout << "[FILTER] " << __func__ << " line is already in the filter check_addr: " << check_addr << " cache_line: " << cache_line;
        std::cout << " quotient: " << quotient << " valid: " << valid[quotient] << " useful: " << useful[quotient] << std::endl;
  ...
  case spp_pgc_pte_va::L2C_DEMAND:
...
void spp_pgc_pte_va::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset,
...
uint32_t spp_pgc_pte_va::GLOBAL_REGISTER::check_entry(offset_type unit_offset)
{
