...
void spp_pgc::prefetcher_initialize()
{
...
void spp_pgc::prefetcher_cycle_operate() {}

uint32_t spp_pgc::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit, bool useful_prefetch, access_type type,
                                           uint32_t metadata_in)
{
  ...
  typename spp_pgc::offset_type::difference_type delta = 0;
  std::vector<typename spp_pgc::offset_type::difference_type> delta_q(intern_->MSHR_SIZE);
  ...
  FILTER.check(addr, spp_pgc::L2C_DEMAND);
...
uint32_t spp_pgc::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr, uint32_t metadata_in)
{
  if constexpr (FILTER_ON) {
    if constexpr (SPP_DEBUG_PRINT) {
      std::cout << std::endl;
    }
    FILTER.check(evicted_addr, spp_pgc::L2C_EVICT);
  }

  return metadata_in;
}

void spp_pgc::prefetcher_final_stats()
{
...
uint64_t spp_pgc::get_hash(uint64_t key)
{
...
void spp_pgc::SIGNATURE_TABLE::read_and_update_sig(champsim::address addr, uint32_t& last_sig, uint32_t& curr_sig, typename offset_type::difference_type& delta)
{
...
void spp_pgc::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
...
void spp_pgc::PATTERN_TABLE::read_pattern(uint32_t curr_sig, std::vector<typename offset_type::difference_type>& delta_q, std::vector<uint32_t>& confidence_q,
...
bool spp_pgc::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
...
void spp_pgc::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset, typename offset_type::difference_type pf_delta)
{
...
uint32_t spp_pgc::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
