                                                     ...
                                                     uint32_t metadata_in)
{
  ...
  typename spp_dev_pgc_grain::offset_type::difference_type delta = 0;
  std::vector<typename spp_dev_pgc_grain::offset_type::difference_type> delta_q(intern_->MSHR_SIZE);
  ...
  do {
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      if (confidence_q[i] >= PF_THRESHOLD) {
        ...
        if (FILTER.check(pf_addr, ((confidence_q[i] >= FILL_THRESHOLD) ? spp_dev_pgc_grain::SPP_L2C_PREFETCH : spp_dev_pgc_grain::SPP_LLC_PREFETCH))) {
          ...
          if (pf_page != page) { // Prefetch request is crossing the physical page boundary
            ...
            if constexpr (GHR_ON) {
              ...
              GHR.update_entry(curr_sig, confidence_q[i], spp_dev_pgc_grain::offset_type{pf_addr}, delta_q[i]);
            }
          }
...
uint32_t spp_dev_pgc_grain::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch, champsim::address evicted_addr,
                                                  uint32_t metadata_in)
{
  if constexpr (FILTER_ON) {
    ...
    FILTER.check(evicted_addr, spp_dev_pgc_grain::L2C_EVICT);
  }

  return metadata_in;
}

void spp_dev_pgc_grain::prefetcher_final_stats()
{
...
uint64_t spp_dev_pgc_grain::get_hash(uint64_t key)
{
...
void spp_dev_pgc_grain::SIGNATURE_TABLE::read_and_update_sig(champsim::address addr, uint32_t& last_sig, uint32_t& curr_sig,
...
void spp_dev_pgc_grain::PATTERN_TABLE::update_pattern(uint32_t last_sig, typename offset_type::difference_type curr_delta)
{
...
void spp_dev_pgc_grain::PATTERN_TABLE::read_pattern(uint32_t curr_sig, std::vector<typename offset_type::difference_type>& delta_q,
...
bool spp_dev_pgc_grain::PREFETCH_FILTER::check(champsim::address check_addr, FILTER_REQUEST filter_request)
{
  ...
  case spp_dev_pgc_grain::SPP_L2C_PREFETCH:
  ...
  case spp_dev_pgc_grain::SPP_LLC_PREFETCH:
    if ((valid[quotient] || useful[quotient]) && remainder_tag[quotient] == remainder) {
      if constexpr (SPP_DEBUG_PRINT) {
        std::cout << "[FILTER] " << __func__ << " line is already in the filter check_addr: " << check_addr << " cache_line: " << cache_line;
        std::cout << " quotient: " << quotient << " valid: " << valid[quotient] << " useful: " << useful[quotient] << std::endl;
  ...
  case spp_dev_pgc_grain::L2C_DEMAND:
  ...
  case spp_dev_pgc_grain::L2C_EVICT:
...
void spp_dev_pgc_grain::GLOBAL_REGISTER::update_entry(uint32_t pf_sig, uint32_t pf_confidence, offset_type pf_offset,
...
uint32_t spp_dev_pgc_grain::GLOBAL_REGISTER::check_entry(offset_type page_offset)
{
