...
uint32_t spp_dev::prefetcher_cache_operate(uint32_t trigger_cpu, champsim::address trigger_paddr, champsim::address trigger_vaddr, champsim::address ip,
                                           bool cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  ...
  if constexpr (SPP_DEBUG_PRINT) {
    std::cout << std::endl << "[ChampSim] " << __func__ << " addr: " << trigger_paddr;
  ...
  FILTER.check(trigger_paddr, spp_dev::L2C_DEMAND);
