...
public:
    template <class T>
    static T set_width(T x, int n) { return ((1ull << n) - 1ull) & x; }
...
{
public:
    spp(int blocks_in_page = 64, int signature_table_size = 256, int pattern_table_size = 512,
        int prefetch_filter_size = 1024, int global_history_register_size = 8, float prefetch_threshold = 0.25)
...
public:
    Cache(int size, int block_size, int num_ways, int page_size, string name)
        : Super(size / block_size, num_ways), spp_module(page_size / block_size), name(name)
    {
